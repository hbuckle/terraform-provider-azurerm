...
func resourceArmFirewallNatRuleCollectionCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	NatRules := expandArmFirewallNatRules(d.Get("rule").([]interface{}))
...
func expandArmFirewallNatRules(inputs []interface{}) []network.AzureFirewallNatRule {
	outputs := make([]network.AzureFirewallNatRule, 0)
...
func flattenFirewallNatRuleCollectionRules(rules *[]network.AzureFirewallNatRule) []map[string]interface{} {
	...
	for _, rule := range *rules {
		...
		if translatedAddress := rule.TranslatedAddress; translatedAddress != nil {
			output["translated_address"] = *translatedAddress
		}
		if translatedPort := rule.TranslatedPort; translatedPort != nil {
			output["translated_port"] = *translatedPort
		}
		...
		if ruleDestinationAddresses := rule.DestinationAddresses; ruleDestinationAddresses != nil {
			output["destination_addresses"] = sliceToSet(*ruleDestinationAddresses)
		}
		if ruleDestinationPorts := rule.DestinationPorts; ruleDestinationPorts != nil {
			output["destination_ports"] = sliceToSet(*ruleDestinationPorts)
		}
